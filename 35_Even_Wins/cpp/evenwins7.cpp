...
//
// Based on evenwins6.cpp. computer_turn improved to play better.
//
...
// The computer's strategy is to use hand-crafted rules for small numbers of
// marbles that (hopefully!) make the best moves. For more marbles, always
// takes 4. Experiments show this is a reasonable strategy, although there's
// no reason to think it's the best. 
// 
// A problem with this strategy is that it is completely deterministic, and so
// the human could learn how the computer works and win games by exploiting
// this determinism.
void computer_turn(Gamestate& state) {
    cout << "It's the computer's turn ...\n";

    // make move
    bool have_even = state.computer_marbles % 2 == 0;
    int n = 0;
    if (have_even) {  // computer has even number of marbles
        switch (state.marbles_in_middle) {
            case 1: n = 1; break;
            case 2: n = 2; break;
            case 3: n = 2; break;
            case 4: n = 4; break;
    ...
    } else {  // computer has odd number of marbles
        switch (state.marbles_in_middle) {
            case 1: n = 1; break;
            case 2: n = 1; break;
            case 3: n = 3; break;
